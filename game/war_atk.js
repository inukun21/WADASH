    ...
    async function cekAFK(x) {
        ...
        if (turn == turnNow && time == timeNow) {
            ...
            if (conn.war[m.chat][turn].hp <= 0) {
                conn.reply(m.chat, `*@${conn.war[m.chat][turn].user.split('@')[0]} sudah mati karena HP (Health Point) habis.*`, null, {
                    contextInfo: {
                        mentionedJid: [conn.war[m.chat][turn].user]
                    }
                })
    ...
    for (let i = 0; i < 10; i++) {
        if (m.sender == conn.war[m.chat][i].user) {
            if (i != conn.war2[m.chat].turn) {
                conn.reply(m.chat, `*Sekarang adalah giliran @${conn.war[m.chat][conn.war2[m.chat].turn].user.split('@')[0]} untuk menyerang.*`, m, {
                    contextInfo: {
                        mentionedJid: [conn.war[m.chat][conn.war2[m.chat].turn].user]
                    }
                })
            ...
            if (conn.war[m.chat][i].user == args[0] && conn.war[m.chat][i].hp > 0) {
                ...
                for (let i = 0; i < 10; i++) {
                    if (conn.war[m.chat][i].user == target) {
                        ...
                        await sleep(2000)
                        if (conn.war[m.chat][i].hp <= 0) conn.reply(m.chat, `*@${target.split(`@`)[0]} sudah mati dalam pertarungan.*`, m, {
                            contextInfo: {
                                mentionedJid: [target]
                            }
                        })
