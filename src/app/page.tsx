        ...
        <div className="space-y-8">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-[var(--muted)] mt-2 text-lg">Monitor bot status and activity</p>
                </div>
                <div className="flex items-center gap-3 w-full md:w-auto">
                    {status === 'connected' ? (
                        ...
                        <button
                            onClick={startBot}
                            className="w-full md:w-auto px-8 py-4 bg-gradient-to-r from-violet-600 to-fuchsia-600 rounded-[2rem] font-bold text-[var(--foreground)] shadow-[0_10px_30px_rgba(124,58,237,0.3)] hover:shadow-[0_15px_40px_rgba(124,58,237,0.4)] hover:scale-105 transition-all duration-300 flex items-center justify-center gap-3"
                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-[var(--card-bg)] backdrop-blur-xl rounded-[2.5rem] p-8 relative group hover:bg-white/10 transition-all duration-500 border border-[var(--border)] overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-[var(--muted)] font-medium mb-1">Status</p>
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className={`text-2xl font-bold tracking-tight ${status === 'connected' ? 'text-emerald-400' : status === 'scan_qr' ? 'text-yellow-400' : 'text-rose-500'}`}>
                ...
                <div className="bg-[var(--card-bg)] backdrop-blur-xl rounded-[2.5rem] p-8 relative group hover:bg-white/10 transition-all duration-500 border border-[var(--border)] overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-[var(--muted)] font-medium mb-1">Session</p>
                        <h3 className={`text-2xl font-bold tracking-tight ${session.exists ? 'text-emerald-400' : 'text-[var(--muted)]'}`}>
                ...
                <div className="bg-[var(--card-bg)] backdrop-blur-xl rounded-[2.5rem] p-8 relative group hover:bg-white/10 transition-all duration-500 border border-[var(--border)] overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-[var(--muted)] font-medium mb-1">Uptime</p>
                        <h3 className="text-2xl font-bold tracking-tight text-[var(--foreground)]">{uptime}</h3>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-[var(--card-bg)] backdrop-blur-xl rounded-[2.5rem] p-8 border border-[var(--border)]">
                    <h3 className="text-xl font-bold tracking-tight text-[var(--foreground)] mb-8 flex items-center gap-3">
                    ...
                    <div className="flex flex-col items-center justify-center py-12 min-h-[350px] bg-black/20 rounded-[2rem] border-2 border-dashed border-[var(--border)] relative overflow-hidden">
                        {/* Background Grid Pattern */}
                        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(#fff 1px, transparent 1px)', backgroundSize: '20px 20px' }}></div>

                        {status === 'connected' ? (
                            <div className="text-center relative z-10">
                                <div className="w-24 h-24 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-6 shadow-[0_0_40px_rgba(16,185,129,0.2)]">
                                    <Wifi className="w-10 h-10 text-emerald-400" />
                                </div>
                                <h4 className="text-2xl font-bold text-[var(--foreground)] mb-2">Connected</h4>
                                <p className="text-[var(--muted)]">Bot is online and ready</p>
                            </div>
                        ) : qr ? (
                            <div className="text-center relative z-10">
                                <div className="bg-white p-4 rounded-3xl mb-6 inline-block shadow-[0_0_40px_rgba(255,255,255,0.1)]">
                                    <img src={qr} alt="QR Code" className="w-56 h-56 rounded-xl" />
                                </div>
                                <p className="text-[var(--muted)] font-medium">Scan with WhatsApp</p>
                            </div>
                        ) : (
                            <div className="text-center relative z-10">
                                <div className="w-24 h-24 bg-[var(--card-bg)] rounded-full flex items-center justify-center mx-auto mb-6 animate-pulse">
                                    <Wifi className="w-10 h-10 text-[var(--muted)]" />
                                </div>
                                <p className="text-[var(--muted)]">Waiting for server...</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-[var(--card-bg)] backdrop-blur-xl rounded-[2.5rem] p-8 border border-[var(--border)]">
                    ...
                    <div className="bg-[var(--background)] rounded-[2rem] p-6 h-[350px] overflow-y-auto font-mono text-sm border border-[var(--border)] shadow-inner">
                        <div className="space-y-3">
                            {logs.map((log, index) => (
                                <div key={index} className="flex gap-3 items-start animate-in fade-in slide-in-from-bottom-2 duration-300">
                                    ...
                                    <div>
                                        <span className={`font-bold ${log.type === 'system' ? 'text-violet-400' :
                                        ...
                                        <span className="text-[var(--foreground)] ml-2">{log.message}</span>
                                        {log.data?.text && (
                                            <span className="text-[var(--muted)] ml-2 italic">"{log.data.text}"</span>
                                        )}
                                    </div>
                                </div>
                            ))}
